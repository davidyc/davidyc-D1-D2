...
{
    //Тут есть вопрос по тестам
    // хватает ли одного теста на каждый метод или нужно делать  по несколько тестов с разными моками(количество файлов , папок)
    ...
    public class UnitTFileSystemVisitor
    {
        ...
        public void GetFilteredElementByNeedStop()
        {
            ...
            FileSystemVisitor fsv = new FileSystemVisitor("c:/", x => x.Contains("New"),
                fileSystem: fileSystem, onlyOneFile: true);
            var expected = new string[]
            {
                @"c:\Folder\New File.txt"
            };
